...
function AddDays(day , days, WeekMask ) {
  ...
  FullWeeks = Math.floor(days / WorkDaysPerWeek);
  ...
  dout = moment().add(day , (FullWeeks * 7 + LastPartial));
  return dout;
  /* end date is number
   weeks * 7 plus
...
/* AddWorkDays(calendar_id,start_date,number_work_days,WeekMask,
 end_date)
 This procedure adds the specified number of working days to the passed in
 start date.  */
...
function WorkDaysBetween(startdt,enddt,WeekMask) {
    ...
    while (CurDt <= TargetDt) {
        /* TODO - Really have to rework this section */
    if (WMask.substr(to_number(to_char(CurDt, 'D')), 1) == 'Y') {
    }
    NumWork = NumWork + 1;
    }
    CurDt = CurDt +1;
